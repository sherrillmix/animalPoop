...
##Last modified: Wed Aug 26, 2015  01:00PM
##Created by: scott
##Summary: Read in OTUs and compare between species

source("~/scripts/R/dna.R")

targets<-read.csv('targetFiles.csv',stringsAsFactors=FALSE)
weights<-read.csv('targetWeights.csv',stringsAsFactors=FALSE) #rough estimates from wikipedia and  http://www.sciencemag.org/content/332/6032/970/suppl/DC1
rownames(weights)<-weights$name
targets<-cbind(targets,weights[targets$name,-1])
targets$otu<-sub('.fastq.gz$','.out',targets$fastq)
...
targets$rareN<-sapply(otuSums,rareEquation,10000)
...
pdf('weight_vs_otu.pdf',height=4,width=4)
	par(mar=c(2.8,3.6,.2,.4))
	plot(targets$weight,targets$rareN,log='xy',xlab='',ylab='Number of OTUs (rarefied to 10,000 reads)',las=1,mgp=c(2.7,.5,0),xaxt='n',pch=21,cex=1.2,bg='#00000011',col='#000000CC')#,pch=pchs,bg=bgs)
	axis(2,4:20*100,rep('',17),tcl=-.2)
	title(xlab='Approximate weight (kg)',mgp=c(1.7,1,0))
	axis(1,c(1,10,100,1000,5000),mgp=c(1,.5,0))
	text(targets$weight,targets$rareN*.98,targets$name,cex=.4,col='#00000044',adj=c(.5,1))
dev.off()
